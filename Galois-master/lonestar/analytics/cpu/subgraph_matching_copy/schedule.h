...
public:
    ...
    Schedule(const std::string _adj_mat, int _size, Graph& complete, uint64_t max_degree);
    ~Schedule();
    ...
    void restricts_generate(const std::string cur_adj_mat, std::vector<std::vector<std::pair<int, int>>> &restricts, Graph& complete, uint64_t max_degree);
