...
//            int size_after_restrict = std::lower_bound(vset.get_data_ptr().begin(), vset.get_data_ptr().begin() + vset.get_size(), min_vertex) - vset.get_data_ptr().begin();
    ...
    for (int &i = ii; i < loop_size; ++i) {
        ...
        for (int prefix_id = schedule.get_last(depth); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
            vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, graph.getData(vertex).adj_node, graph.getData(vertex).degree, prefix_id, vertex);
    ...
    galois::do_all(
        ...
        [&](uint64_t vertex) {
            for (int prefix_id = schedule.get_last(0); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
                vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, graph.getData(vertex).adj_node, graph.getData(vertex).degree, prefix_id);
            }
            subtraction_set.push_back(vertex);
...
Schedule::Schedule(const Pattern& pattern, bool &is_pattern_valid, int performance_modeling_type, int restricts_type, bool use_in_exclusion_optimize, Graph& complete, uint64_t _max_degree, int v_cnt, unsigned int e_cnt, long long tri_cnt) {
    ...
    if (performance_modeling_type != 0) {
        ...
        // 更新schedule的adj_mat为最优匹配顺序
        const std::string pattern_adj_mat = pattern.get_adj_mat();
        for (int i = 0; i < size; ++i)
            for (int j = 0; j < size; ++j)
