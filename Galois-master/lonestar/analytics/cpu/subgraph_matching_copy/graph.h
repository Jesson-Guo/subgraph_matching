...
public:
    ...
    std::vector<int> edge;            // edges
    std::vector<unsigned int> vertex; // v_i's neighbor is in edge[ vertex[i], vertex[i+1]-1]
...
private:
    ...
    void pattern_matching_func(const Schedule &schedule, std::vector<VertexSet> vertex_set, VertexSet &subtraction_set, long long &local_ans, int depth, bool clique = false);

    void pattern_matching_aggressive_func(const Schedule &schedule, std::vector<VertexSet> vertex_set, VertexSet &subtraction_set, VertexSet &tmp_set, long long &local_ans, int depth);

    //this function will be defined at code generation
    void unfold_pattern_matching_aggressive_func(const Schedule &schedule, std::vector<VertexSet> vertex_set, VertexSet &subtraction_set, VertexSet &tmp_set, long long &local_ans, int depth);
};
...
void Graph::pattern_matching_func(const Schedule &schedule, std::vector<VertexSet> vertex_set, VertexSet &subtraction_set, long long &local_ans, int depth, bool clique) {
    ...
    for (int i = 0; i < loop_size; ++i) {
        ...
        for (int prefix_id = schedule.get_last(depth); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
            std::vector<int> input_data(edge.begin() + l, edge.begin() + r);
            vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, input_data, (int)r - l, prefix_id, vertex, clique);
...
#pragma omp for schedule(dynamic) nowait
        for (int vertex = 0; vertex < v_cnt; ++vertex) {
            ...
            for (int prefix_id = schedule.get_last(0); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
                std::vector<int> input_data(edge.begin() + l, edge.begin() + r);
                vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, input_data, (int)r - l, prefix_id);
            }
            //subtraction_set.insert_ans_sort(vertex);
            subtraction_set.push_back(vertex);
...
void Graph::pattern_matching_aggressive_func(const Schedule &schedule, std::vector<VertexSet> vertex_set, VertexSet &subtraction_set, VertexSet &tmp_set, long long &local_ans, int depth) {
    int loop_set_prefix_id = schedule.get_loop_set_prefix_id(depth); // @@@
    int loop_size = vertex_set[loop_set_prefix_id].get_size();
    if (loop_size <= 0)
        return;
...
*/
    ...
    for (int &i = ii; i < loop_size; ++i) {
        ...
        for (int prefix_id = schedule.get_last(depth); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
            std::vector<int> input_data(edge.begin() + l, edge.begin() + r);
            vertex_set[prefix_id].build_vertex_set(schedule, vertex_set, input_data, (int)r - l, prefix_id, vertex);
            if (vertex_set[prefix_id].get_size() == 0) {
                is_zero = true;
                break;
            }
