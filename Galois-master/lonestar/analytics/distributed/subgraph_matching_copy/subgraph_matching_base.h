...
void pattern_matching_aggressive_func(Graph& graph, const Schedule &schedule, std::vector<VertexSet> vertex_set, VertexSet &subtraction_set, VertexSet &tmp_set, uint64_t max_degree, long long &local_ans, int depth) {
...
//            int size_after_restrict = std::lower_bound(vset.get_data_ptr().begin(), vset.get_data_ptr().begin() + vset.get_size(), min_vertex) - vset.get_data_ptr().begin();
    ...
    for (int &i = ii; i < loop_size; ++i) {
        ...
        for (int prefix_id = schedule.get_last(depth); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
            vertex_set[prefix_id].build_vertex_set(
                schedule, vertex_set, graph.getData(graph.getLID(vertex)).adj_node, 
...
long long pattern_matching(Graph& graph, const Schedule &schedule, uint64_t max_degree) {
    ...
    global_ans.reset();
    std::vector<VertexSet> vertex_set = std::vector<VertexSet>(schedule.get_total_prefix_num());
    VertexSet subtraction_set;
    VertexSet tmp_set;
    subtraction_set.init(graph.globalSize());
    ...
    galois::do_all(
        ...
        [&](uint64_t vertex) {
            for (int prefix_id = schedule.get_last(0); prefix_id != -1; prefix_id = schedule.get_next(prefix_id)) {
                vertex_set[prefix_id].build_vertex_set(
                    schedule, vertex_set, graph.getData(vertex).adj_node, graph.getData(vertex).degree, prefix_id);
            }
            std::cout << "\n ---------------- " << graph.getGID(vertex) << " ------------------\n";
...
Schedule::Schedule(const Pattern& pattern, bool &is_pattern_valid, int performance_modeling_type, int restricts_type, bool use_in_exclusion_optimize, Graph& complete, uint64_t _max_degree, int v_cnt, unsigned int e_cnt, long long tri_cnt) {
    ...
    adj_mat = pattern.get_adj_mat_ptr();
    ...
    // best_order[] Finally, we use best_order[] to relocate adj_mat
    if (performance_modeling_type != 0) {
        ...
        const std::string pattern_adj_mat = pattern.get_adj_mat_ptr();
        for (int i = 0; i < size; ++i)
            for (int j = 0; j < size; ++j)
...
Schedule::Schedule(const std::string _adj_mat, int _size, Graph& complete, uint64_t _max_degree) {
    size = _size;
    adj_mat = _adj_mat;
    max_degree = _max_degree;
...
int Schedule::find_father_prefix(int data_size, std::vector<int> data) {
    if (data_size == 0)
        return -1;
...
void Schedule::init_in_exclusion_optimize() {
    ...
    id.clear();
    in_exclusion_val.clear();
}

void Schedule::get_in_exclusion_optimize_group(int depth, std::vector<int>& id, int id_cnt, std::vector<int>& in_exclusion_val) {
    if (depth == in_exclusion_optimize_num) {
        std::vector<int> size_in_ex(id_cnt);
        for (int i = 0; i < id_cnt; ++i)
            size_in_ex[i] = 0;
        for (int i = 0; i < in_exclusion_optimize_num; ++i)
            size_in_ex[ id[i] ] ++;
        int val[2];
        val[0] = in_exclusion_val[ size_in_ex[0] * 2 - 2 ];
        val[1] = in_exclusion_val[ size_in_ex[0] * 2 - 1 ];
        for (int i = 1; i < id_cnt; ++i) {
            int tmp0 = val[0];
            int tmp1 = val[1];

            val[0] = tmp0 * in_exclusion_val[ size_in_ex[i] * 2 - 2] + tmp1 * in_exclusion_val[ size_in_ex[i] * 2 - 1];
            val[1] = tmp0 * in_exclusion_val[ size_in_ex[i] * 2 - 1] + tmp1 * in_exclusion_val[ size_in_ex[i] * 2 - 2];
        }
...
void Schedule::restricts_generate(const std::string cur_adj_mat, std::vector<std::vector<std::pair<int,int>>> &restricts, Graph& complete, uint64_t max_degree) {
